...
public class GUIUpdater {
  ...
  public void updateBoardWithPawns(JButton[][] chessBoardGUIButtons) {
    for (int k = 0; k < chessBoardGUIButtons.length; k++) {
      ...
      JButton silverSquare =
        chessBoardGUIButtons[k][chessBoardGUIButtons.length - 2];
  ...
  public void updateBoardWithSpecialPieces(JButton[][] chessBoardGUIButtons) {
    for (int l = 0, m = 0; l < chessBoardGUIButtons.length; l++, m++) {
      ...
      ImageIcon goldIcon = new ImageIcon(
        ChessSprites.SPRITES_IN_ORDER.get(l + m)
      );
      goldSquare.setIcon(goldIcon);

      JButton silverSquare =
        chessBoardGUIButtons[l][chessBoardGUIButtons.length - 1];
      ImageIcon silverIcon = new ImageIcon(
        ChessSprites.SPRITES_IN_ORDER.get(l + m + 1)
      );
      silverSquare.setIcon(silverIcon);
    }
  }
  ...
  public void updateBoardWithNewMove(
    ...
    JButton moveFromSquare =
      chessBoardGUIButtons[newMove.getMoveFromColumn()][newMove.getMoveFromRow()];
    moveFromSquare.setIcon(null);

    ImageIcon pieceImage = new ImageIcon(
      ChessSprites.getCorrespondingImageFromPiece(pieceBeingMoved)
    );
    JButton moveToSquare =
      chessBoardGUIButtons[newMove.getMoveToColumn()][newMove.getMoveToRow()];
